...
class JqueryValidationService {
	...
	static final DEFAULT_ERROR_MESSAGE_CODES_MAP = [
			...
			notEqualTo: "default.not.equal.to.message",
			equal     : "default.equal.message",
			equalTo   : "default.equal.to.message",
	...
	private String getMessage(def constraint, Class validatableClass, def args, Locale locale) {
		...
		if (!grailsApplication.config.jqueryValidationUi.useLegacyMessageCodes && constraint.respondsTo('rejectValue')) {
			...
			message = validationErrors.fieldErrors[0].codes.findResult {
				messageSource.getMessage(it, argArray, null, locale)
			}
		} else {
			...
			if (javaScriptConstraint) {
				switch (constraint.name) {
					...
					case "notEqual":
						...
						break
					case "notEqualTo":
						javaScriptConstraintCode = "${javaScriptConstraint}: ${constrainedProperty.propertyType == Date ? "new Date(${constrainedProperty.notEqualTo.time})" : "'${constrainedProperty.notEqualTo}'"}"
						break
					case "equal":
						javaScriptConstraintCode = "${javaScriptConstraint}: ${constrainedProperty.propertyType == Date ? "new Date(${constrainedProperty.equal.time})" : "'${constrainedProperty.equal}'"}"
						break
					case "equalTo":
						javaScriptConstraintCode = "${javaScriptConstraint}: ${constrainedProperty.propertyType == Date ? "new Date(${constrainedProperty.equalTo.time})" : "'${constrainedProperty.equalTo}'"}"
						break
					case "range":
